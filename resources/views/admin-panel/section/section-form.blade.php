...
@section('title')
{{ __('Sections') }} {{ __( ucfirst(request()->segment(count(request()->segments())))) }}
@endsection
                                ...
                                <div class="d-flex align-items-baseline flex-wrap mr-5">
                                    ...
                                    <ul
                                        ...
                                        <li class="breadcrumb-item text-muted">
                                            <a href="{{ route('section.index', [ app()->getlocale(), 'all' ]) }}"
                                                class="text-muted">{{ __('Sections') }}</a>
                                        </li>

                                        <li class="breadcrumb-item text-muted">
                                            <a href="{{ route('section.index', [ app()->getlocale(), $sectionType ]) }}"
                                                class="text-muted">
                                                {{ __( ucfirst(Request::segment(3)) . ' Sections') }}
                                            </a>
                                        </li>
                                    </ul>
                                    <!--end::Breadcrumb-->
                            ...
                            <div class="card card-custom">
                                <div class="card-header flex-wrap py-5">
                                    <div class="card-title">
                                        <h3 class="card-label">{{ __('Sections') }}
                                ...
                                @if($section->id)
                                <form
                                    action="{{ route(Request::segment(4) . '.update', [ app()->getlocale(), $sectionType, $section->id ] ) }}"
                                    method="post" enctype="multipart/form-data">
                                    @csrf
                                    @method('PUT')
                                    @else
                                    <form
                                        action="{{ route(Request::segment(4) . '.store', [ app()->getlocale(), $sectionType ] ) }}"
                                        method="post" enctype="multipart/form-data">
                                        @csrf
                                        @endif
                                                ...
                                                <div class="row">
                                                    ...
                                                    <div class="col-4">
                                                        <div class="form-group">
                                                            ...
                                                            <input type="text"
                                                                ...
                                                                value="{{ $section->{ 'name_' . $lang } ?? '' }}{{ request()->segment(count(request()->segments())) == 'create' ? old('name_' . $lang) : '' }}" />
                                                    ...
                                                    <div class="col-4">
                                                        <div class="form-group">
                                                            ...
                                                            <select class="form-control" id="exampleSelect1" name="section_id">
                                                                <option value="">---{{ __('unselected') }}---
                                                                </option>
                                                                @foreach($parentSections as $parentSection)
                                                                <option value="{{ $parentSection->id }}" {{ $parentSection->id == $section->section_id ? 'selected=selected' : '' }}>{{ $parentSection->id }}) {{ $parentSection->{ 'name_' . app()->getlocale() } }}</option>
                                                                @endforeach
                                                            </select>
                                                        </div>
                                                    </div>
                                        ...
                                        <div class="card-footer d-flex justify-content-between">
                                            ...
                                            <button type="submit"
                                                title="{{ $section->id ? __('Edit') : __('Create') }}"
                                                class="btn {{ $section->id ? 'btn-warning' : 'btn-primary' }} font-weight-bolder">
                                                <span class="svg-icon svg-icon-md">
                                                    @if($section->id)
                                                    <span class="svg-icon svg-icon-md {{ $section->id ? 'svg-icon-dark' : '' }}">
                                                ...
                                                <span class="{{ $section->id ? 'text-dark' : '' }}">
                                                    {{ $section->id ? __('Edit') : __('Create') }}
                                                </span>
                                            </button>
                                        </div>
                                    </form>
