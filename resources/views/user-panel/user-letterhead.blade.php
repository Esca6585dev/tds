    ...
    <div class="section__profile__body">
      ...
      <div class="section__profile__cart">
          <div class="section__container__cart__wrapper">
            <form action="{{ route('profile.letterhead.edit', app()->getlocale() ) }}" method="post" enctype="multipart/form-data">
                @csrf
                ...
                <div class="row hide" id="div__buttons">
                    <div class="col letterhead__buttons pt-10">
                        <button type="button" class="btn p-10-20 ma-10 w-100p" onclick="inputDisable()">{{ __('Cancel') }}</button>
                        
                        <div class="btn p-10-20 ma-10 w-100p bg-white text-align-center">
                            <label>{{ __('Emblem') }} {{ __('or') }} {{ __('Logo') }}</label>
                            <input type="file" class="form-control" name="image" id="companyLogo" />
                        </div>

                        <button type="sumbit" class="btn p-10-20 ma-10 w-100p bg-goldenrod">{{ __('Save') }}</button>
                    </div>
                </div>
            </form>
