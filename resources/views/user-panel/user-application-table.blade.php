@if(count($applications))
<div>
    <table>
        <thead>
            <tr>
                <th>ID</th>
                <th>{{ __('First Name') }} {{ __('and') }} {{ __('Last Name') }}</th>
                <th class="hide-1200">{{ __('IP Address') }}</th>
                <th class="hide-700">{{ __('Address') }}</th>
                <th>{{ __('Phone number') }}</th>
                <th class="hide-700">{{ __('Date') }}</th>
                <th>{{ __('Section') }}</th>
                <th>{{ __('Code number') }}</th>
                <th>{{ __('Download') }}</th>
                <th>{{ __('View') }}</th>
            </tr>
        </thead>
        <tbody>
            @foreach($applications as $application)
            <tr>
                <td>{{ $loop->iteration }}</td>
                <td>{{ $application->user->first_name }} {{ $application->user->last_name }}</td>
                <td class="hide-1200">{{ $application->ip_address }}</td>
                <td class="hide-700">{{ $application->user->address }}</td>
                <td>+993 {{ $application->user->phone_number }}</td>
                <td class="hide-700">{{ \Carbon::parse($application->created_at)->locale(config('app.faker_locales.' . app()->getlocale() ))->isoFormat('LLLL') }}</td>
                <td>{{ $application->bolum }}</td>
                <td>{{ $application->code_number }}</td>
                <td>
                    <a href="{{ asset($application->file) }}">
                        <svg width="16px" height="16px" viewBox="0 0 24 24" version="1.1" xmlns="http://www.w3.org/2000/svg" xmlns:xlink="http://www.w3.org/1999/xlink">
                            <title>{{ __('Download') }}</title>
                            <g id="Stockholm-icons-/-Files-/-Download" stroke="none" stroke-width="1" fill="none" fill-rule="evenodd">
                                <rect id="bound" x="0" y="0" width="24" height="24"></rect>
                                <path d="M2,13 C2,12.5 2.5,12 3,12 C3.5,12 4,12.5 4,13 C4,13.3333333 4,15 4,18 C4,19.1045695 4.8954305,20 6,20 L18,20 C19.1045695,20 20,19.1045695 20,18 L20,13 C20,12.4477153 20.4477153,12 21,12 C21.5522847,12 22,12.4477153 22,13 L22,18 C22,20.209139 20.209139,22 18,22 L6,22 C3.790861,22 2,20.209139 2,18 C2,15 2,13.3333333 2,13 Z" id="Path-57" fill="#000000" fill-rule="nonzero" opacity="0.6"></path>
                                <rect id="Rectangle" fill="#000000" opacity="0.6" transform="translate(12.000000, 8.000000) rotate(-180.000000) translate(-12.000000, -8.000000) " x="11" y="1" width="2" height="14" rx="1"></rect>
                                <path d="M7.70710678,15.7071068 C7.31658249,16.0976311 6.68341751,16.0976311 6.29289322,15.7071068 C5.90236893,15.3165825 5.90236893,14.6834175 6.29289322,14.2928932 L11.2928932,9.29289322 C11.6689749,8.91681153 12.2736364,8.90091039 12.6689647,9.25670585 L17.6689647,13.7567059 C18.0794748,14.1261649 18.1127532,14.7584547 17.7432941,15.1689647 C17.3738351,15.5794748 16.7415453,15.6127532 16.3310353,15.2432941 L12.0362375,11.3779761 L7.70710678,15.7071068 Z" id="Path-102" fill="#000000" fill-rule="nonzero" transform="translate(12.000004, 12.499999) rotate(-180.000000) translate(-12.000004, -12.499999) "></path>
                            </g>
                        </svg>
                    </a>
                </td>
                <td>
                    <a href="{{ route('profile.docx', [app()->getlocale(), $application->code_number]) }}">
                        <i class="fa fa-eye"></i>
                    </a>
                </td>
            </tr>
            @endforeach
        </tbody>
    </table>
    {{ $applications->links('user-panel.pagination') }}
</div>
@else
<div class="no-data">
    {{ __('No Data') }}
</div>
@endif
