<span class="dtr-data d-flex">
    <a href="{{ route(Request::segment(3) . '.show', [ app()->getlocale(), $text->id ] ) }}"
    ...
    <form action="{{ route(Request::segment(3) . '.destroy', [ app()->getlocale(), $text->id ] ) }}"
        method="post">
        @method('delete')
        @csrf

        <button type="button" class="btn btn-sm btn-clean btn-icon" title="{{ __('Delete') }}" data-toggle="modal"
            data-target="#exampleModalSizeSm-{{$text->id}}">
        ...
        <div class="example-preview">
            <!--begin::Modal-->
            <div class="modal fade" id="exampleModalSizeSm-{{$text->id}}" tabindex="-1" aria-labelledby="exampleModalSizeSm-{{$text->id}}"
