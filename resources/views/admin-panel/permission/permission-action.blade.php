<span class="dtr-data d-flex">
    <a href="{{ route(Request::segment(3) . '.show', [ app()->getlocale(), $permission->id ] ) }}"
    ...
    <a href="{{ route(Request::segment(3) . '.edit', [ app()->getlocale(), $permission->id ] ) }}"
    ...
    <form action="{{ route(Request::segment(3) . '.destroy', [ app()->getlocale(), $permission->id ] ) }}"
        method="post">
        @method('delete')
        @csrf

        <button type="button" class="btn btn-sm btn-clean btn-icon" title="{{ __('Delete') }}" data-toggle="modal"
            data-target="#exampleModalSizeSm-{{$permission->id}}">
        ...
        <div class="example-preview">
            <!--begin::Modal-->
            <div class="modal fade" id="exampleModalSizeSm-{{$permission->id}}" tabindex="-1" aria-labelledby="exampleModalSizeSm-{{$permission->id}}"
