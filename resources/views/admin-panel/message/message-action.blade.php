<span class="dtr-data d-flex">
    <a href="{{ route(Request::segment(3) . '.show', [ app()->getlocale(), $message->id ] ) }}"
    ...
    <a href="{{ route(Request::segment(3) . '.edit', [ app()->getlocale(), $message->id ] ) }}"
    ...
    <form action="{{ route(Request::segment(3) . '.destroy', [ app()->getlocale(), $message->id ] ) }}"
        method="post">
        @method('delete')
        @csrf

        <button type="button" class="btn btn-sm btn-clean btn-icon" title="{{ __('Delete') }}" data-toggle="modal"
            data-target="#exampleModalSizeSm-{{$message->id}}">
        ...
        <div class="example-preview">
            <!--begin::Modal-->
            <div class="modal fade" id="exampleModalSizeSm-{{$message->id}}" tabindex="-1" aria-labelledby="exampleModalSizeSm-{{$message->id}}"
