<span class="dtr-data d-flex">
    <a href="{{ route(Request::segment(3) . '.show', [ app()->getlocale(), $standart->id ] ) }}"
    ...
    <a href="{{ route(Request::segment(3) . '.edit', [ app()->getlocale(), $standart->id ] ) }}"
    ...
    <form action="{{ route(Request::segment(3) . '.destroy', [ app()->getlocale(), $standart->id ] ) }}"
        method="post">
        @method('delete')
        @csrf

        <button type="button" class="btn btn-sm btn-clean btn-icon" title="{{ __('Delete') }}" data-toggle="modal"
            data-target="#exampleModalSizeSm-{{$standart->id}}">
        ...
        <div class="example-preview">
            <!--begin::Modal-->
            <div class="modal fade" id="exampleModalSizeSm-{{$standart->id}}" tabindex="-1" aria-labelledby="exampleModalSizeSm-{{$standart->id}}"
