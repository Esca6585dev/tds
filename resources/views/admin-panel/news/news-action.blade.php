<span class="dtr-data d-flex">
    <a href="{{ route(Request::segment(3) . '.show', [ app()->getlocale(), $row->id ] ) }}"
    ...
    <a href="{{ route(Request::segment(3) . '.edit', [ app()->getlocale(), $row->id ] ) }}"
    ...
    <form action="{{ route(Request::segment(3) . '.destroy', [ app()->getlocale(), $row->id ] ) }}"
        method="post">
        @method('delete')
        @csrf

        <button type="button" class="btn btn-sm btn-clean btn-icon" title="{{ __('Delete') }}" data-toggle="modal"
            data-target="#exampleModalSizeSm-{{$row->id}}">
        ...
        <div class="example-preview">
            <!--begin::Modal-->
            <div class="modal fade" id="exampleModalSizeSm-{{$row->id}}" tabindex="-1"
                aria-labelledby="exampleModalSizeSm-{{$row->id}}" style="display: none;" aria-hidden="true">
